...
Each row group is a collection of column chunks, each of which contain a the same number of rows. Each column chunk is a series of data pages. Since each column chunk is a just a series of pages, and each row group is a series of column chunks, the final file simply looks like a series of pages from each column interleaved with each other. We're able to make sense of it all by using the metadata to identify the offset and size of each row group and column chunk.
